...
if [[ -f $NARCHOS_INSTALL_LOG_FILE ]] && grep -q "Total:" "$NARCHOS_INSTALL_LOG_FILE" 2>/dev/null; then
  echo
  TOTAL_TIME=$(tail -n 20 "$NARCHOS_INSTALL_LOG_FILE" | grep "^Total:" | sed 's/^Total:[[:space:]]*//')
...
if sudo test -f /etc/sudoers.d/99-narchos-installer; then
  sudo rm -f /etc/sudoers.d/99-narchos-installer &>/dev/null
  echo
...
if gum confirm --padding "0 0 0 $((PADDING_LEFT + 32))" --show-help=false --default --affirmative "Reboot Now" --negative "" ""; then
  ...
  if [[ -n "${NARCHOS_CHROOT_INSTALL:-}" ]]; then
    touch /var/tmp/narchos-install-completed
    exit 0
  else
