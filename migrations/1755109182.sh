...
# Users can still opt-in to Cloudflare DNS using: narchos-setup-dns cloudflare
...
if [ -f /etc/systemd/resolved.conf ]; then
  ...
  if [ -f /etc/systemd/network/99-narchos-dns.network ]; then
    sudo rm -f /etc/systemd/network/99-narchos-dns.network
  ...
  echo "To use Cloudflare DNS, run: narchos-setup-dns Cloudflare"
fi