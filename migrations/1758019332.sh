...
narchos-refresh-config uwsm/default
narchos-refresh-config uwsm/env
narchos-state set relaunch-required
...
if grep -q "bindd = SUPER, N, Neovim" ~/.config/hypr/bindings.conf; then
  sed -i '/SUPER, N, Neovim, exec/ c\bindd = SUPER, N, Editor, exec, narchos-launch-editor' ~/.config/hypr/bindings.conf
fi
